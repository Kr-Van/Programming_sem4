...
def fib_gen():
    """
    Сопрограмма генерирует ряд Фибоначчи
    """
    ...
    while True:
        ...
        if num == 0 or isinstance(num, str) or num is None:
            res = None
            continue
...
class FibonacciIter:
    ...
    __next__ : берёт элемент с индексом self.idx -> проверяет, больше ли он 0 -> проверяет, входит ли в ряд фибоначи -> при исчерпании индексов поднимается StopIteration
    """
...
class SimpleFibonacciIter:
    """
    Простой итератор по числам Фибоначчи

    При инициализации принимает collection и создает list в переменной self.data, в который записывает подходящие элементы

    __getitem__ : пробует получить элемент из self.data: если успешно - возвращает, а если нет - поднимает IndexError
    """
